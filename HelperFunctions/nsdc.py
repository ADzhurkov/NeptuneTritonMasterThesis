...
def J2000_2_ECLIPJ2000(RA_J2000,DEC_J2000): 
    ...
    '''
    time = time_representation.julian_day_to_seconds_since_epoch(DateTime.from_python_datetime(datetime.datetime(2000,1,1,12,0,0)).to_julian_day())   #Dummy values, no effect but required input #time_representation.calendar_date_to_julian_day(datetime.datetime(2000,1,1,12,0,0))
...
def ECLIPJ2000_2_J2000(RA_ECLIPJ2000,DEC_ECLIPJ2000):
    ...
    '''
    time = time_representation.julian_day_to_seconds_since_epoch(time_representation.calendar_date_to_julian_day(datetime.datetime(2000,1,1,12,0,0)))   #Dummy values, no effect but required input
...
def JD_to_tudat(values,time_entry,time_scale):
    ...
    '''
    epoch = time_representation.datetime_to_tudat(time_representation.julian_day_to_calendar_date(Time(values[time_entry],scale=time_scale,format='jd').tdb.value))
    return(epoch)
...
def date_to_tudat(values,time_entry,time_scale,amount_time_entries,time_delta):
    ...
    if int(amount_time_entries)==1:
        ...
        date = time_representation.DateTime(int(values[time_entry]),int(values[time_entry+1]),floor(values[time_entry+2]),floor((values[time_entry+2]%1)*24-time_delta),floor((values[time_entry+2]*24)%1*60),floor((values[time_entry+2]*24*60)%1*60*10**6)/10**6)
    elif int(amount_time_entries)==4:
        date = time_representation.DateTime(int(values[time_entry]),int(values[time_entry+1]),int(values[time_entry+2]),floor(values[time_entry+3]-time_delta),floor((values[time_entry+3]%1)*60),floor((values[time_entry+3]*60)%1*60*10**6)/10**6)
    elif int(amount_time_entries)==5:
        date = time_representation.DateTime(int(values[time_entry]),int(values[time_entry+1]),int(values[time_entry+2]),int(values[time_entry+3]-time_delta),floor(values[time_entry+4]),floor((values[time_entry+4]%1)*60*10**6)/10**6)
    elif int(amount_time_entries)==6:
        date = time_representation.DateTime(int(values[time_entry]),int(values[time_entry+1]),int(values[time_entry+2]),int(values[time_entry+3]-time_delta),int(values[time_entry+4]),values[time_entry+5])

    #Conversion from utc to tdb to julian day to seconds
    #epoch = time_representation.time_from_julian_day(Time(date.julian_day(),scale=time_scale,format='jd').tdb.value)  #Seconds since 2000
    ...
    if date.year <1960: #utc only starts at 1960, before sacrificing precision for functionality
        ...
        epoch = time_representation.date_time_from_epoch(spice.convert_date_string_to_ephemeris_time(string))
    else:    
        converter = time_representation.default_time_scale_converter()
        TimeScales = time_representation.TimeScales
        epoch = time_representation.date_time_from_epoch(converter.convert_time(getattr(TimeScales, f"{time_scale}_scale", None),TimeScales.tdb_scale,date.epoch()))
    return epoch
...
def MJD_to_tudat(values,time_entry,time_scale):
    ...
    '''        
    epoch = time_representation.datetime_to_tudat(time_representation.julian_day_to_calendar_date(time_representation.modified_julian_day_to_julian_day(Time(values[time_entry],scale=time_scale,format='mjd').tdb.value)))  #Seconds since 2000
    return epoch
...
def minsinceJD_to_tudat(values,time_entry,time_scale,JD):
    ...
    '''
    epoch = time_representation.datetime_to_tudat(time_representation.julian_day_to_calendar_date(Time(JD + values[time_entry]/(60*24),scale=time_scale,format='jd').tdb.value))
    return epoch
...
def save_nsdc_data_to_csv(times, RADEC, moons, observatories, studyname, diflist, output_folder):
    ...
    for moon in unique_moons:
        ...
        for observatory in unique_observatories:
            ...
            if not moon.isalpha():
                moon = str(int(moon))
            with open(output_folder + moon + '_' + str(observatory) + '_' + studyname + '.csv', 'w',
                      newline='') as csvfile:
