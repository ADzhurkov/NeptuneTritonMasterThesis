...
from tudatpy.numerical_simulation import environment_setup, propagation_setup, environment,Time
...
class TudatOrbitRunner:
    ...
    def initial_state_from_spice(self, scenario: Scenario) -> np.ndarray:
        return spice.get_body_cartesian_state_at_epoch(
            ...
            ephemeris_time=scenario.start_epoch.to_float()
        )
    ...
    def run(self, scenario: Scenario):
        ...
        integ = propagation_setup.integrator.runge_kutta_fixed_step(
            Time(scenario.step), coefficient_set=propagation_setup.integrator.CoefficientSets.rk_4
        )
        term = propagation_setup.propagator.time_termination(scenario.end_epoch.to_float())
        ...
        states = result2array(sim.propagation_results.state_history_float)
        deps   = result2array(sim.propagation_results.dependent_variable_history_float)
        return states, deps
